...
const PriceRangeWearables = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                setLoading(true);
                const response = await fetch(`${API_BASE_URL}/api/price-range-smartwatches/${range}`);
        ...
        <Box sx={styles.gradientBg}>
            <Container sx={styles.container}>
                <Typography sx={{ ...styles.resultsText, mb: { xs: 2, sm: 4 } }}>
                    Wearables in {range} <span>({filteredProducts.length} found)</span>
                    </Typography>
                        ...
                        {filteredProducts.length > 0 ? (
                            ...
                            {filteredProducts.map((product) => (
                                ...
                                <Typography variant="h6" sx={{ fontSize: { xs: '1.1rem', sm: '1.25rem' } }}>
                                            No wearables found in this price range
                                        </Typography>
...
export default PriceRangeWearables;
